...
describe VR::Reducer::Departementales::Annee2021 do
  subject { described_class.new({ name: 'departementales' }, mapper) }
  ...
  before do
    stub_request(:get, 'https://example.com/tour-1')
      .to_return(status: 200, body: file_fixture('departementales_2021/tour-1.xlsx'), headers: {})
    stub_request(:get, 'https://example.com/tour-2')
      .to_return(status: 200, body: file_fixture('departementales_2021/tour-2.xlsx'), headers: {})
  end

  describe 'new' do
    it 'turns a content array into a hash' do
      expect(subject.content).not_to be_empty
      expect(subject.content[:data].length).to be(20)
